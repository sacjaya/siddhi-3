...
public class EventConverter {
     ...
     */
    public EventConverter(MetaStreamEvent metaStreamEvent, StreamDefinition defaultDefinition) {
        ...
        this.outDataSize = metaStreamEvent.getOutData().size();

        int size = metaStreamEvent.getBeforeWindowData().size() + metaStreamEvent.getAfterWindowData().size() + metaStreamEvent.getOutData().size();
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (j == 0) {
                ...
                currentDataList = metaStreamEvent.getOutData();
            }
     ...
     * @return
     */
    public InnerStreamEvent constructInnerStreamEvent(Object[] data) {

        InnerStreamEvent innerStreamEvent = new InnerStreamEvent();
        ...
        for (ConverterMetaElement converterMetaElement : converterHelper) {
            if (converterMetaElement.getToPosition()[0] == Constants.BEFORE_WINDOW_DATA_INDEX) {
                beforeWindowData[converterMetaElement.getToPosition()[1]] = data[converterMetaElement.getFromPosition()];
            } else if (converterMetaElement.getToPosition()[0] == Constants.AFTER_WINDOW_DATA_INDEX) {
                afterWindowData[converterMetaElement.getToPosition()[1]] = data[converterMetaElement.getFromPosition()];
            } else if (converterMetaElement.getToPosition()[0] == Constants.OUT_DATA_INDEX) {
                outData[converterMetaElement.getToPosition()[1]] = data[converterMetaElement.getFromPosition()];
            }
        }

        innerStreamEvent.setBeforeWindowData(beforeWindowData);
        innerStreamEvent.setOnAfterWindowData(afterWindowData);
        innerStreamEvent.setData(outData);
